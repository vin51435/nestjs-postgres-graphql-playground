...
export class PostsResolver {
  ...
  @ResolveField(() => User, {
    description: 'Optimized Author field resolver using DataLoader (Batched SQL Query - NO N+1 Problem)',
  })
  ...
  @ResolveField(() => User, {
    description: 'Unoptimized Author field resolver (Triggers 1 SQL query per post - Demonstrates N+1 Problem)',
  })
  async naiveAuthor(@Parent() post: Post): Promise<User> {
    console.log(`[Naive N+1 Resolver] Executing individual SQL SELECT for authorId: ${post.authorId}`);
